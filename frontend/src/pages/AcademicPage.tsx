...
function AcademicPage() {
    ...
    useEffect(() => {
        if(templateData && filename) {
            navigate('/create_template', {state: {templateData: templateData, filename: filename}})
        }
                            ...
                            <Box width="100%" style={{padding: "0 3.5%"}}>
                                ...
                                >
                                    <FileInput
                                        ...
                                        required
                                    />
                                    {isError && <Paragraph color="red" margin="10px 0 -10px" textAlign="center" style={{width: "100%", maxWidth: "100%"}}>Ошибка загрузки файла</Paragraph>}
                                    <Button disabled={isLoading} label="Отправить" type="submit" margin={"medium"} primary style={{...orangeFillButtonStyle, width: "100%", height: "50px", margin: "24px 0"}}/>
                                </Form>
                            </Box>
            </Box>
                </>
